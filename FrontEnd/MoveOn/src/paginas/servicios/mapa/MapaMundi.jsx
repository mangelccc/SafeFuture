    ...
    click: async (e) => {
      ...
      if (formValues) {
        try {
          ...
          // Determinar clave ID correcta de la respuesta (id, id_pais, _id...)
          const newId = data.id ?? data.id_pais ?? data._id;
...
export default function MapaMundi({ userId }) {
    ...
    (async () => {
      ...
      try {
        ...
        const arrayData = Array.isArray(rawData)
          ? rawData
          : rawData.paises ?? rawData.data ?? rawData.results;

        const mapped = arrayData.map(loc => ({
  ...
  const handleAddNewLocation = newLocation => setLocations(prev => [...prev, newLocation]);
  ...
  const handleDelete = async locId => {
    ...
    if (isConfirmed) {
      try {
        const res = await fetch(`${API_URL}/${locId}`, { method: 'DELETE' });
              ...
              <Popup className="bg-black1 rounded-2xl p-4">
                <div className="space-y-1">
                  ...
                  <p className="text-black1 text-xs italic">
                    Usuario: {usuarios.filter(u => u.id_usuario === loc.userId)[0]?.nombre || loc.userId}
                  </p>
                  {loc.userId === usuario.id_usuario && (
                    <button onClick={() => handleDelete(loc.id)} className="mt-2 text-red-500 underline">
                      Eliminar ubicación
                    </button>
                  )}
                </div>
